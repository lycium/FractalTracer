...
inline vec3f generateColour(int x, int y, int frame, int pass, int xres, int yres, int frames, Scene & scene) noexcept
{
...
#if 1 // Depth of field
	const real focal_dist = length(cam_pos) * 0.8f;
	const real lens_radius = 0.01f;
	...
	while (true)
	{
		...
		{
			const vec3f sky_up = vec3f{ 53, 112, 128 } * (1.0f / 255) * 1.0f;
			const vec3f sky_hz = vec3f{ 182, 175, 157 } * (1.0f / 255) * 1.0f;
		...
		vec3f albedo;
		if (nearest_hit_obj->use_fresnel)
		{
			const real r0 = nearest_hit_obj->r0;
			...
			albedo = (sample_specular) ? 0.95f : nearest_hit_obj->albedo;
		}
		else
		{
			sample_specular = false;
			albedo = nearest_hit_obj->albedo;
		}
